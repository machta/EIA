...
void LUOptimizedSimple::solve(float* A, float* b, float* x, int n, int N)
{
	...
	for (int j = 0; j < n; j++)
	{
		...
		{
			// Evaluate U.
			float sum = vectorSum(i, j, i, A, N, U, UN, U0);
		...
		{
			// Evaluate L.
			float sum = vectorSum(i, j, j, A, N, U, UN, U0);
...
float LUOptimizedSimple::vectorSum(int i, int j, int elements, float* A, int N, float* U, int UN, int U0)
{
	float sum = 0;
